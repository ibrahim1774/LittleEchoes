...
export function OnboardingFlow3() {
  ...
  // After the intro, hand off to account creation. The child is captured during
  // setup, then the user lands on the paywall (/onboarding-3/plans). The flag tells
  // AddChild to route to that paywall instead of straight to /home.
  async function startAccountFlow() {
    sessionStorage.setItem('le_onboarding3', 'true');
    if (state.user) {
      const parent = await getParent();
      navigate(parent ? '/setup/child' : '/setup/parent');
    } else {
      navigate('/signup');
    }
  }
